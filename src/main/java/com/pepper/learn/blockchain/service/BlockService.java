package com.pepper.learn.blockchain.service;
...
import com.pepper.learn.blockchain.model.Block;
import com.pepper.learn.blockchain.model.Transaction;
import com.pepper.learn.blockchain.model.TransactionInput;
import com.pepper.learn.blockchain.model.TransactionOutput;
import com.pepper.learn.blockchain.model.Wallet;
import com.pepper.learn.blockchain.pwdutil.EncryptUtil;
...
public class BlockService {
	...
	private String calculateHash(String previousHash, List<Transaction> currentTransactions, int nonce) {
		return EncryptUtil.SHA256(previousHash + JSON.toJSONString(currentTransactions) + nonce);
	}
	...
	public Transaction createCoinbaseTx(String toAddress) {
		...
		return new Transaction(EncryptUtil.UUID(), txIn, txOut);
	}
	...
	public Transaction createTransaction(Wallet senderWallet, Wallet recipientWallet, int amount) {
		...
		Transaction transaction = new Transaction(EncryptUtil.UUID(), txIn, txOut);
	...
	private List<Transaction> findUnspentTransactions(String address) {
		...
		for (Block block : blockChain) {
			...
			for (Transaction tx : transactions) {
				if (address.equals(EncryptUtil.MD5(tx.getTxOut().getPublicKeyHash()))) {
