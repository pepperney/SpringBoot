package com.pepper.learn.blockchain.model;

import com.alibaba.fastjson.JSON;
import com.pepper.learn.blockchain.pwdutil.EncryptUtil;
import com.pepper.learn.blockchain.pwdutil.RSAUtil;
...
public class Transaction {
	...
	public void sign(String privateKey, Transaction prevTx) {
		...
		String sign = "";
		try {
			sign = RSAUtil.sign(txClone.hash().getBytes(), privateKey);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean verify(Transaction prevTx) {
		...
		try {
			result = RSAUtil.verify(txClone.hash().getBytes(), txIn.getPublicKey(), txIn.getSignature());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	 ...
	 */
	public String hash() {
		return EncryptUtil.SHA256(JSON.toJSONString(this));
	}
