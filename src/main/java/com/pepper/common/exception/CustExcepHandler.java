...
public class CustExcepHandler {
	...
	private static final Logger log = LoggerFactory.getLogger(CustExcepHandler.class);
	...
	public void msdExceptionHandle(CustomException ex, HttpServletRequest request, HttpServletResponse response) {
		...
		// token异常
		if (Code.TOKEN_ERROR.getCode().equals(ex.getCode())) {
	...
	public void exceptionHandle(Exception e, HttpServletRequest request, HttpServletResponse response) {
		log.error("======>系统异常", e);
		dealExceptionResponse(SYSTEM_ERROR_CODE, new CommonResp(Code.SYSTEM_ERROR), request, response);
